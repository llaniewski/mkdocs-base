...
The source code of completed 'D2Q9 Single Relaxation Time' tutorial is given [here](https://google.com) # TODO
...
As per the [FD wave equation tutorial](https://github.com/CFD-GO/TCLB/wiki/Tutorial---FD-Wave-equation), we want to set up a folder named 'myPseudoPotential' in the _/models/_ folder within TCLB. Additionally, the generic file structure needs to be created consisting of _conf.mk, Dynamics.c, Dynamics.R_. 
...
From the above, we have all the variables we need to implement the LBM. The first step is to initialise the lattice over the required domain, this is incorporated as part of the dynamics that are occurring in the model and must be incorporated into the _Dynamics.c_ file within the **Init()** function.
...
<CLBConfig output="output/">
        ...
        <Solve Iterations="50000" output="output/"> <VTK Iterations="500"/> </Solve>
</CLBConfig>
...
With the model and set-up files created, we can now look to make and run `myPseudoPotential`. 
...
```bash
mpirun -np 8 CLB/myPseudoPotential/main example/multiphase/pseudo_potential/my_pp_config.xml
```
