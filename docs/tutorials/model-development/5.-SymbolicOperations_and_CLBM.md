...
Next, go `tutorials` folder and open the `tutorial_cm_eq_derivation.py` script.
Run it.
...
We are going to generate the missing functions:
...
To do so, open a prepared `tutorial_cm_collision.py` script.
